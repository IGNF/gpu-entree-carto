...
| `@opacity` | `(id, opacity)` | Curseur opacité 0–100 % (désactivé si `forceOpacity` dans LAYER_CONFIG → 100 % fixe) |
| `@toggle-grayscale` | `(id)` | Bascule affichage niveaux de gris / couleurs |
...
- Barre horizontale : **œil** (`ri-eye-line` / `ri-eye-off-line`), **poubelle** (`ri-delete-bin-line`), **contraste** (`ri-contrast-fill`, bascule gris / couleurs, état actif surligné), curseur DSFR (`fr-range`) + pourcentage.
