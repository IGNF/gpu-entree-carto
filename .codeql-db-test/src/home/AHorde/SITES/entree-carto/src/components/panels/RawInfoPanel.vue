...
<template>
  <section class="ec-raw-info" aria-label="Données brutes">
    <h2 class="ec-raw-info__title">Attributs</h2>
    <ul v-if="entries.length" class="ec-raw-info__list">
      <li v-for="row in entries" :key="row.key">
        <span class="ec-raw-info__key">{{ row.key }}</span>
        <span class="ec-raw-info__val">{{ row.value }}</span>
      </li>
    </ul>
    <div v-else class="ec-raw-info__placeholder">
